...
public class BST<Key extends Comparable<Key>> {
    ...
    public double averageDepth() {
        return 1.0 * internalPathLength() / size();
    }

    /* Returns the internal path length of the BST. */
    public int internalPathLength() {
        return internalPathLengthInOrder(root, 0);
    }

    private int internalPathLengthInOrder(Node x, int depth) {
        if (x == null) {
            return 0;
        }
        int leftPathLength = internalPathLengthInOrder(x.left, depth + 1);
        int rightPathLength = internalPathLengthInOrder(x.right, depth + 1);
        return leftPathLength + depth + rightPathLength;
    }
