    ...
    public boolean isSeparable() {
        ...
        for (String label : g.labels()) {
            ...
            while (!queue.isEmpty()) {
                ...
                for (String neighbor : g.neighbors(cur)) {
                    if (!colors.containsKey(neighbor)) {                    // if neighbor hasn't been colored
                        colors.put(neighbor, -colors.get(cur));             // color it with a different color
                        queue.offer(neighbor);
                    } else if (colors.get(neighbor) == colors.get(cur)) {  // if two adjacent nodes have the same color
                        return false;
                    }
