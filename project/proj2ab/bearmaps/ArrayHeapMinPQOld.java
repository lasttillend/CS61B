...
public class ArrayHeapMinPQOld<T> implements ExtrinsicMinPQ<T> {
    ...
    public ArrayHeapMinPQOld(int initCapacity) {
        minHeap = new ArrayHeapMinPQOld.PriorityNode[initCapacity + 1];
//        minHeap = (PriorityNode[]) new Object[initCapacity + 1];  // doesn't work
        items = new TreeMap<>();
        n = 0;
    }

    public ArrayHeapMinPQOld() {
        this(1);
    }
    ...
    private void resize(int capacity) {
        PriorityNode[] temp = new ArrayHeapMinPQOld.PriorityNode[capacity];
        for (int i = 1; i <= n; i++) {
            temp[i] = minHeap[i];
        }
        minHeap = temp;
